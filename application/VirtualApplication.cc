...
myDataPacket* VirtualApplication::createMyDataPacket(double* dataArrayToSend, unsigned int seqNum, int numElements)
{
    myDataPacket *newPacket = new myDataPacket("App data array packet", APPLICATION_PACKET);
    newPacket->setMyDataArrayArraySize(numElements);
    for (int k=0; k<numElements; k++) {
        newPacket->setMyDataArray(k, dataArrayToSend[k]);
    }
    newPacket->setSequenceNumber(seqNum);
    return newPacket;
}
/*
ApplicationPacket* VirtualApplication::createGenericDataPacketM(vector<double> data, unsigned int seqNum, int size)
{
    ApplicationPacket *newPacket = new ApplicationPacket("App generic packet", APPLICATION_PACKET);
    newPacket->setDataM(data);
    newPacket->setSequenceNumber(seqNum);
    if (size > 0) newPacket->setByteLength(size);
    return newPacket;
}*/
